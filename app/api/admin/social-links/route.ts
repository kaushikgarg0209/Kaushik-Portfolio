...
export async function GET() {
  ...
  const result = await runAdminQuery("socialLinks", () =>
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error(err);
    return jsonError("Failed to create social link", 500);
  }
}
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const parsed = socialLinkSchema.safeParse(rest);
    if (!parsed.success) {
      return validationErrorResponse(parsed.error);
    }
    ...
    console.error(err);
    return jsonError("Failed to update social link", 500);
  }
}
...
export async function DELETE(request: Request) {
  ...
  const result = await runAdminQuery("socialLinks.delete", () =>
    db.delete(socialLinks).where(eq(socialLinks.id, id)),
  );
  if (result.error) return result.error;
  revalidatePortfolio();
